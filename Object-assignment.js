...
const house = {
  ...
  getHouseInfo: function () {
    console.log(`House Name: ${house.houseName},House Number: ${
      house.houseNumber
    },
 Name of Residents: ${house.houseResidentsNames.join(", ")}`);
  ...
  getHouseName: () => {
    console.log(
      `we have ${house.houseName} for rent at ${house.houseLocation} but the color is ${house.housePaint} going in for ${house.housePrice}.`
    );
  },
};
//Remove one resident
house.houseResidentsNames.unshift("Mr wood"), house.getHouseName();

// Add another resident to the end of the array
house.houseResidentsNames.push("Mr Kwame Adu"), console.log(house);
